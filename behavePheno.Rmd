...
idCBARQ <- merge(idPV, cbnf, by.x="Animal_Name", by.y="dog",
...
phenoC <- select(idCBARQ,
                 ## covariates
                 Animal_Name:Animal_ID, breed, humanrole,
...
dataFrom <- c("PV, C", "C", "C", rep("PV", times=119))
...
rownames(dataCBARQ) <- seq(from =1, to = 122, by = 1)
...
idBCL <- merge(idPV, bcl2, by.x="Animal_Name", by.y="dog", all=FALSE)
...
### dim(phenoB)
### head(fields2, 15)
dataFrom2 <- c("PV, C", "C", "C", "C" ,"C", rep("PV", 49))
### names(phenoB)
